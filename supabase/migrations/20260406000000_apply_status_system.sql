-- ─────────────────────────────────────────────────────────────────────────────
-- Migration: Apply status system + product extended columns
-- Run this in your Supabase SQL editor if you see errors like:
--   "column products.default_views does not exist"
-- ─────────────────────────────────────────────────────────────────────────────

-- 1. STATUS CATEGORY ENUM
DO $$ BEGIN
  CREATE TYPE status_category AS ENUM ('not_started', 'active', 'done', 'closed');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

-- 2. PRODUCT STATUSES TABLE
...
-- 5. updated_at trigger for product_statuses
DO $$ BEGIN
...
-- 6. ADD EXTENDED COLUMNS TO PRODUCTS (safe — IF NOT EXISTS)
...
-- 7. ADD status_id TO WORKSPACE FEATURES
ALTER TABLE public.workspace_features
  ADD COLUMN IF NOT EXISTS status_id UUID REFERENCES public.product_statuses(id) ON DELETE SET NULL;

-- 8. FUNCTION: auto-create 7 default statuses on product insert
...
-- 9. BACKFILL: create statuses for products that don't have them yet
DO $$
DECLARE p record;
DECLARE v_org_id UUID;
BEGIN
  FOR p IN
    ...
    WHERE NOT EXISTS (SELECT 1 FROM public.product_statuses ps WHERE ps.product_id = pr.id)
  LOOP
...
-- 10. BACKFILL: set status_id on existing workspace_features using old status enum
DO $$
DECLARE f record;
DECLARE s_id UUID;
DECLARE s_name TEXT;
